---
name: pptx
description: "用在任何需要处理演示文稿文件作为主要输入或输出时。覆盖：从零创建新的 .pptx 演示文稿，或者基于大纲/文档生成 PPT；编辑和重构现有 .pptx；读取并提取演示文稿中的文本与结构；将演示文稿导出为 PDF 或按页生成 PNG 图片用于预览、缩略图或视觉检查。触发条件：用户提到 .pptx 文件、PPT、演示文稿、幻灯片、汇报材料、路演稿、做汇报、生成演示文稿、导出 PDF、导出图片、PPT 转 PDF、ppt 导出图片、把这些 slides 转成 PDF 等。"
---
